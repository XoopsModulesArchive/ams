...
include "header.php";
// SEO Url Rewrite
$url = XOOPS_URL.'/'.$GLOBALS['xoopsModuleConfig']['baseurl'].'/audience'.$GLOBALS['xoopsModuleConfig']['endofurl'];
if (!strpos($url, $_SERVER['REQUEST_URI'])&&$GLOBALS['xoopsModuleConfig']['htaccess']==true&&empty($_POST)) {
	header( "HTTP/1.1 301 Moved Permanently" ); 
	header('Location: '.$url);
	exit(0);
}
$xoopsOption['template_main'] = 'ams_audience.html';	
include_once XOOPS_ROOT_PATH."/header.php";
$criteria = new CriteriaCompo();
$criteria->add( new Criteria('audience_title', '', '!='));
$criteria->setSort("audience_title");
...
{			
	...
	{	
		...
		$xoopsTpl->append('audience', array('id' => $audience_arr[$i]->getVar('audience_id'), 'audience' => $audience_arr[$i]->getVar('audience_audience')));								
	}
}
