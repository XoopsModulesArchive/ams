...
function newSubmissions()
{
    ...
    if ( count( $storyarray ) > 0 )
    {
        ...
        foreach( $storyarray as $newstory )
        {
            ...
            {
                echo "<a href='main.php?op=edit&amp;storyid=" . $newstory -> storyid() . "'>" . _AD_NOSUBJECT . "</a>\n";
            }
            else
            {
                echo "&nbsp;<a href='../submit.php?op=edit&amp;storyid=" . $newstory -> storyid() . "'>" . $title . "</a>\n";
            }
            echo "</td><td align='center' class='nw'>" . formatTimestamp( $newstory -> created(), 's' ) . "</td><td align='center'><a href='" . XOOPS_URL . "/userinfo.php?uid=" . $newstory -> uid() . "'>" . $newstory -> uname . "</a></td><td align='right'><a href='main.php?op=delete&amp;storyid=" . $newstory -> storyid() . "'>" . _AMS_AM_DELETE . "</a></td></tr>\n";
        }
        echo "</table></div>\n";
        echo"</td></tr></table>";
        echo "<br />";
...
function lastStories()
{
    ...
    foreach( $storyarray as $storyid => $eachstory ) {
        ...
        echo "
            ...
            <td align='center'><a href='../submit.php?op=edit&amp;storyid=" . $eachstory -> storyid() . "'>" . _AMS_AM_EDIT . "</a>-<a href='main.php?op=delete&amp;storyid=" . $eachstory -> storyid() . "'>" . _AMS_AM_DELETE . "</a>";
        echo "</td></tr>\n";
    }
	}
    echo "</table><br /></div>";
...
function topicsmanager()
{
    ...
    echo $index_admin->addNavigation('main.php?op=topicsmanager') ;
    ...
    if($totaltopics > 0 ) {
        ...
        foreach ($topics_arr as $thisTopic) {
            ...
            if ($i > $start && $i - $start <= $xoopsModuleConfig['storycountadmin']) {
                $linkedit = XOOPS_URL . '/modules/'.$xoopsModule->dirname() . '/admin/main.php?op=topicsmanager&amp;topic_id=' . $thisTopic->topic_id();
                $linkdelete = XOOPS_URL . '/modules/'.$xoopsModule->dirname() . '/admin/main.php?op=delTopic&amp;topic_id=' . $thisTopic->topic_id();
    ...
    $sform = new XoopsThemeForm($formlabel, "topicform", XOOPS_URL.'/modules/'.$xoopsModule->getVar('dirname').'/admin/main.php', 'post');
...
function modTopicS()
{
    ...
    {
        redirect_header( 'main.php?op=topicsmanager', 2, _AMS_AM_ADD_TOPIC_ERROR1 );
    }
    $xt -> setTopicPid( $_POST['topic_pid'] );
    if ( empty( $_POST['topic_title'] ) )
    {
        redirect_header( "main.php?op=topicsmanager", 2, _AMS_AM_ERRORTOPICNAME );
    }
    ...
    redirect_header( 'main.php?op=topicsmanager', 1, _AMS_AM_DBUPDATED );
    exit();
}
...
function delTopic()
{
    if (!isset($_REQUEST['topic_id']) || $_REQUEST['topic_id'] == 0) {
        redirect_header('main.php?op=topicsmanager', 3, _AMS_AM_NOTOPICSELECTED);
    }
    ...
    if (!isset($_POST['ok']))
    {
        ...
        xoops_confirm( array( 'op' => 'delTopic', 'topic_id' => intval( $_GET['topic_id'] ), 'ok' => 1 ), 'main.php', _AMS_AM_WAYSYWTDTTAL . '<br />' . $xt->topic_title('S'));
    }
    else
    {
...
function addTopic()
{
    ...
    {
        redirect_header( 'main.php?op=topicsmanager', 2, _AMS_AM_ADD_TOPIC_ERROR );
        exit();
    }
...
function audienceForm($id = 0) {
    ...
    include_once XOOPS_ROOT_PATH."/class/xoopsformloader.php";
    $aform = new XoopsThemeForm(_AMS_AM_MANAGEAUDIENCES, "audienceform", 'main.php', 'post');
    if ($id > 0) {
...
switch ( $op )
{
    ...
    xoops_cp_header();
    $index_admin = new ModuleAdmin() ;
    echo $index_admin->addNavigation('main.php?op=newarticle') ;
    ...
    xoops_cp_header();
    $index_admin = new ModuleAdmin() ;
    echo $index_admin->addNavigation('main.php?op=audience') ;
    ...
    if ( !empty( $_POST['ok'] ) )
    {
        if (empty($storyid))
        {
            redirect_header( 'main.php?op=newarticle', 2, _AMS_AM_EMPTYNODELETE );
            exit();
        }
        ...
        redirect_header( 'main.php?op=newarticle', 1, _AMS_AM_DBUPDATED );
        exit();
    }
    else
    {
        $story = new AmsStory($storyid);
        echo "<h4>" . _AMS_AM_CONFIG . "</h4>";
        xoops_confirm( array( 'op' => 'delete', 'storyid' => $storyid, 'ok' => 1 ), 'main.php', _AMS_AM_RUSUREDEL .'<br />' . $story->title());
    }
    include "admin_footer.php";
    break;
    ...
    if ($_GET['audienceid'] == 1) {
        redirect_header('main.php?op=audience', 2, _AMS_AM_CANNOTDELETEDEFAULTAUDIENCE);
    }
    $audience_handler =& xoops_getmodulehandler('audience', 'AMS');
    ...
    if ($storycount > 0) {
        xoops_confirm( array( 'op' => 'delaudienceok', 'audienceid' => $_GET['audienceid'], 'newaudience' => $newaudiences), 'main.php', $thisaudience->getVar('audience') . "<br />". sprintf(_AMS_AM_AUDIENCEHASSTORIES, $storycount));
    }
    else {
        xoops_confirm( array( 'op' => 'delaudienceok', 'audienceid' => $_GET['audienceid'], 'newaudience' => 1), 'main.php', $thisaudience->getVar('audience') . "<br />". _AMS_AM_RUSUREDELAUDIENCE);
    }
    break;
    ...
    if ($audience_handler->delete($thisaudience, $_POST['newaudience'])) {
        redirect_header('main.php?op=audience', 3, _AMS_AM_AUDIENCEDELETED);
    }
    else {
        redirect_header('main.php?op=audience', 3, _AMS_AM_ERROR_AUDIENCENOTDELETED);
    }
    break;

    case "reorder":
    if (!isset($_POST['weight']) || !is_array($_POST['weight']) || (count($_POST['weight']) == 0)) {
        header("location:main.php?op=topicsmanager");
    }
    ...
    if ($errors) {
        redirect_header('main.php?op=topicsmanager', 3, _AMS_AM_ERROR_REORDERERROR);
    }
    else {
        redirect_header('main.php?op=topicsmanager', 3, _AMS_AM_REORDERSUCCESSFUL);
    }
    break;
}
