...
include_once XOOPS_ROOT_PATH . '/modules/AMS/class/xoopstree.php';
...
class XoopsTopicAms
{
    ...
    function XoopsTopicAms($table, $topicid = 0)
    {
    ...
    function store()
    {
        ...
        if ($this->use_permission == true) {
            ...
            $xt = new XoopsTreeAMS($this->table, "topic_id", "topic_pid");
    ...
    function getFirstChildTopics()
    {
        ...
        if (is_array($topic_arr) && count($topic_arr)) {
            foreach ($topic_arr as $topic) {
                $ret[] = new XoopsTopicAms($this->table, $topic);
            }
        }
        return $ret;
    }
