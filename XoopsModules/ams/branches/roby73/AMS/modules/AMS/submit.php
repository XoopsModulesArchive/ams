...
include_once XOOPS_ROOT_PATH.'/modules/'.$xoopsModule->getVar('dirname').'/include/date.php';
...
case "post":
	...
	if ($approveprivilege) {
	    ...
	    if ( !empty( $_POST['autodate'] )) {
	        $pubdate = convert_date($_POST['publish_date']['date']) + $_POST['publish_date']['time'];
	    ...
	    if ( !empty( $_POST['autoexpdate'] )) {
	        $expiry_date = convert_date($_POST['expiry_date']['date']) + $_POST['expiry_date']['time'];
	        $offset = $xoopsUser -> timezone() - $xoopsConfig['server_TZ'];
