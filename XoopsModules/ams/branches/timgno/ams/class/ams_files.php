 ...
 */
	...
	class ams_files extends XoopsObject
	{ 
		//Constructor
		function __construct()
		{
			$this->XoopsObject();
			$this->initVar("fileid",XOBJ_DTYPE_INT,null,false,8);
			$this->initVar("filerealname",XOBJ_DTYPE_TXTBOX,null,false);
			$this->initVar("storyid",XOBJ_DTYPE_INT,null,false,8);
			$this->initVar("date",XOBJ_DTYPE_INT,null,false,10);
			$this->initVar("mimetype",XOBJ_DTYPE_TXTBOX,null,false);
			$this->initVar("downloadname",XOBJ_DTYPE_TXTBOX,null,false);
		...
		function getForm($action = false)
		{
			...
			$title = $this->isNew() ? sprintf(_AM_AMS_FILES_ADD) : sprintf(_AM_AMS_FILES_EDIT);
			...
			$form->addElement(new XoopsFormFile(_AM_AMS_FILEREALNAME, "filerealname", $xoopsModuleConfig["filerealname_size"]), true);	
			...
			$form->addElement(new XoopsFormTextDateSelect(_AM_AMS_DATE, "date", "", $this->getVar("date")));
			
			$XoopsFormSelectBoxHandler =& xoops_getModuleHandler("ams_XoopsFormSelectBox", "ams");
			$XoopsFormSelectBox_select = new XoopsFormSelect(_AM_AMS_MIMETYPE, "mimetype", $this->getVar("mimetype"));
			$XoopsFormSelectBox_select->addOptionArray($XoopsFormSelectBoxHandler->getList());
			$form->addElement($XoopsFormSelectBox_select, true);
			$form->addElement(new XoopsFormText(_AM_AMS_DOWNLOADNAME, "downloadname", 50, 255, $this->getVar("downloadname")), true);
			
			$NullHandler =& xoops_getModuleHandler("ams_Null", "ams");
			$Null_select = new XoopsFormSelect(_AM_AMS_COUNTER, "counter", $this->getVar("counter"));
			$Null_select->addOptionArray($NullHandler->getList());
			$form->addElement($Null_select, false);
			
			$form->addElement(new XoopsFormHidden("op", "save_files"));
			$form->addElement(new XoopsFormButton("", "submit", _SUBMIT, "submit"));
			$form->display();
			return $form;
		}
	}
	class amsams_filesHandler extends XoopsPersistableObjectHandler 
	{

		function __construct(&$db) 
		{
			parent::__construct($db, "ams_files", "ams_files", "files_id", "filerealname");
		}
