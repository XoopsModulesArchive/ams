...
class ams_article extends XoopsObject
{ 
	...
	function __construct()
	{
		...
		$this->initVar("rating",XOBJ_DTYPE_INT,null,false,3);
	...
	function ams_article()
	{
		$this->__construct();
		$this->setFriendlyUrl('',1,$this->getVar("storyid"),0);
	}
	...
	{		
		...
		$title = $this->isNew() ? sprintf(_AM_AMS_ARTICLE_ADD) : sprintf(_AM_AMS_ARTICLE_EDIT);
		...
		$form->addElement(new XoopsFormText(_AM_AMS_TITLE, "title", 50, 255, $this->getVar("title")), true);			
		...
		//Show topic image?
		$topic_img = new XoopsFormRadio(_AM_AMS_TOPICDISPLAY, 'topicdisplay', $this->topicdisplay());
		$topic_img->addOption(0, _AM_AMS_NONE);
		$topic_img->addOption(1, _AM_AMS_TOPIC);
		$topic_img->addOption(2, _AM_AMS_AUTHOR);
		$form->addElement($topic_img);
		
		//Select image position
		$posselect = new XoopsFormSelect(_AM_AMS_TOPICALIGN, 'topicalign', $this->topicalign());            
		$posselect->addOption('L', _AM_AMS_LEFT);
		$posselect->addOption('R', _AM_AMS_RIGHT);
		$form->addElement($posselect);	 
		
		$ihome = $this->isNew() ? 1 : $this->getVar("ihome");
		$form->addElement(new XoopsFormRadioYN(_AM_AMS_IHOME, "ihome", $ihome, _YES, _NO), false);
		
		$audienceHandler =& xoops_getModuleHandler("ams_article", "ams");
		$audience_select = new XoopsFormSelect(_AM_AMS_AUDIENCEID, "audienceid", $this->getVar("audienceid"));
		$audience_select->addOptionArray($audienceHandler->getList());
		$form->addElement($audience, true);			
		...
		$editor_configs["editor"] = $GLOBALS['xoopsModuleConfig']["ams_editor"];				
		...
		$editor_configs["height"] = "200px";
		$editor_configs["editor"] = $GLOBALS['xoopsModuleConfig']["ams_editor"];				
		$form->addElement( new XoopsFormEditor(_AM_AMS_BODYTEXT, "bodytext", $editor_configs), false );
		...
		//if ($approveprivilege) {
			$approve_checkbox = new XoopsFormCheckBox('', 'approve', $this->approved);
			$approve_checkbox->addOption(1, _AM_AMS_APPROVE);
			$option_tray->addElement($approve_checkbox);

			$published_checkbox = new XoopsFormCheckBox('', 'autodate',$this->published()?1:0);
			$published_checkbox->addOption(1, _AM_AMS_SETDATETIME);
			$option_tray->addElement($published_checkbox);

			$option_tray->addElement(new XoopsFormDateTime(_AM_AMS_SETDATETIME, 'publish_date', 15, $this->published()));

			$expired_checkbox = new XoopsFormCheckBox('', 'autoexpdate',$this->expired()?1:0);
			$expired_checkbox->addOption(1, _AM_AMS_SETEXPDATETIME);
			$option_tray->addElement($expired_checkbox);

			$option_tray->addElement(new XoopsFormDateTime(_AM_AMS_SETEXPDATETIME, 'expiry_date', 15, $this->expired()));
		//}

		if (is_object($GLOBALS['xoopsUser'])) {
			$notify_checkbox = new XoopsFormCheckBox('', 'notifypub', $this->notifypub());
			$notify_checkbox->addOption(1, _AM_AMS_NOTIFYPUBLISH);
			$option_tray->addElement($notify_checkbox);
			if ($GLOBALS['xoopsUser']->isAdmin($GLOBALS['xoopsModule']->getVar('mid'))) {
				$nohtml_checkbox = new XoopsFormCheckBox('', 'nohtml', $this->nohtml());
		...
		/*$notifypub = $this->isNew() ? 1 : $this->getVar("notifypub");
		$check_notifypub = new XoopsFormCheckBox(_AM_AMS_NOTIFYPUB, "notifypub", $notifypub);
		$check_notifypub->addOption(1, " ");
		$form->addElement($check_notifypub);*/
		$form->addElement(new XoopsFormText(_AM_AMS_STORY_TYPE, "story_type", 50, 255, $this->getVar("story_type")), true);
		
		$change_radio = new XoopsFormRadio(_AM_AMS_MAJOR, 'change', $this->change);
		$change_radio->addOption(0, _AM_AMS_NOVERSIONCHANGE);
		$change_radio->addOption(1, _AM_AMS_VERSION);
		$change_radio->addOption(2, _AM_AMS_REVISION);
		$change_radio->addOption(3, _AM_AMS_MINOR);
		$change_radio->addOption(4, _AM_AMS_AUTO);
		$change_radio->setDescription(_AM_AMS_VERSIONDESC);
		$change_radio->setValue(4);
		$form->addElement($change_radio);
		$form->addElement(new XoopsFormRadioYN(_AM_AMS_SWITCHAUTHOR." (".$this->uname().")", 'newauthor', 0));			
		...
		$form->addElement(new XoopsFormHidden("op", "save_article"));
	...
	function title($format = "Show")
    {
        ...
        case "Show":
        case "Edit":
            $title = $myts->htmlSpecialChars($this->getVar("title"));
            break;
        case "Preview":
        case "InForm":
            $title = $myts->htmlSpecialChars($myts->stripSlashesGPC($this->getVar("title")));
            break;
        }
        return $title;
    }
	...
	function hometext($format="Show")
    {
        ...
        switch ( $format ) {
            case "Show":
            $hometext = $myts->displayTarea($this->getVar("hometext"),$html,$smiley,$xcodes);
            break;
            case "Edit":
            $hometext = $myts->htmlSpecialChars($this->getVar("hometext"));
            break;
            case "Preview":
            $hometext = $myts->previewTarea($this->getVar("hometext"),$html,$smiley,$xcodes);
            break;
            case "InForm":
            $hometext = $myts->htmlSpecialChars( $myts->stripSlashesGPC($this->getVar("hometext")));
            break;
            case "N":
            $hometext = stripslashes($this->getVar("hometext"));
            break;
        }
        return $hometext;
    }
    ...
    function bodytext($format="Show")
    {
        ...
        switch ( $format ) {
            case "Show":
            $bodytext = $myts->displayTarea($this->getVar("bodytext"),$html,$smiley,$xcodes);
            break;
            case "Edit":
            $bodytext = $myts->htmlSpecialChars($this->getVar("bodytext"));
            break;
            case "Preview":
            $bodytext = $myts->previewTarea($this->getVar("bodytext"),$html,$smiley, $xcodes);
            break;
            case "InForm":
            $bodytext = $myts->htmlSpecialChars($myts->stripSlashesGPC($this->getVar("bodytext")));
            break;
            case "N":
            $bodytext = stripslashes($this->getVar("bodytext"));
            break;
        }
        return $bodytext;
    }
	...
	function getAllPublished($limit=0, $start=0, $checkRight = false, $topic=0, $ihome=0, $asobject=true, $order = 'published', $ids = false)
    {
        ...
        if ( $topic != 0 ) {
            ...
            if ($checkRight) {                
                if (!isset($GLOBALS['xoopsModule']) || $GLOBALS['xoopsModule']->getVar('dirname') != "ams") {
                    $module_handler =& xoops_gethandler('module');
                    $newsModule =& $module_handler->getByDirname('ams');
                }
                else {
                    $newsModule =& $GLOBALS['xoopsModule'];
                }
    ...
    */
    ...
    function uname($users = false)
    {
        ...
        if (!isset($GLOBALS['xoopsModule']) || $GLOBALS['xoopsModule']->getVar('dirname') != "ams") {
            ...
            $option = $GLOBALS['xoopsModuleConfig']['displayname'];
        }
    ...
    function getRating() 
	{
		$this->rating = $this->getVar("rating");
	...
	{        
        ...
        if(!$GLOBALS['xoopsUser'])
        {
            if ($GLOBALS['xoopsModuleConfig']['anonymous_vote'] == 0) {
        ...
        {
            $ratinguser = $GLOBALS['xoopsUser']->uid();
	...
	{       
        ...
        if ( trim($bodytext) != '' ) {
            if (false != $pagenav) {
                ...
                if ($story_pages > 1 && $storypage != -1) {
                    ...
                    $pagenav = new PageNav($story_pages, 1, $storypage, 'page', 'storyid='.$this->storyid,$this->friendlyurl_enable,$this->friendlyurl);
        ...
        $story['mail_link'] = 'mailto:?subject='.sprintf(_AMS_NW_INTARTICLE,$GLOBALS['xoopsConfig']['sitename']).'&amp;body='.sprintf(_AMS_NW_INTARTFOUND, $GLOBALS['xoopsConfig']['sitename']).':  '.XOOPS_URL.'/modules/ams/article.php?storyid='.$this->storyid();
    ...
    function version() 
	{
	    $this->revision = $this->getVar("revision");
        $revision = $this->revision < 10 ? "0".$this->revision : $this->revision;
        if ($this->revisionminor > 0) {
            $revisionminor = $this->revisionminor < 10 ? ".0".$this->revisionminor : ".".$this->revisionminor;
        }
    ...
    function getBanner() 
	{
	    $this->banner = $this->getVar("banner");
        if ($this->banner) {
            return $this->banner;
        }
        else {
    ...
    */
	...
	function topicid()
	{
	    $this->topicid = $this->getVar("topicid");
		return $this->topicid;
	}
	...
	function counter()
	{
	    $this->counter = $this->getVar("counter");
		return $this->counter;
	}

	function created()
	{
	    $this->created = $this->getVar("created");
		return $this->created;
	}

	function published()
	{
	    $this->published = $this->getVar("published");
		return $this->published;
	}

	function expired()
	{
	    $this->expired = $this->getVar("expired");
		return $this->expired;
	}

	function hostname()
	{
	    $this->hostname = $this->getVar("hostname");
		return $this->hostname;
	}

	function storyid()
	{
	    $this->storyid = $this->getVar("storyid");
		return $this->storyid;
	}

	function nohtml()
	{
	    $this->nohtml = $this->getVar("nohtml");
		return $this->nohtml;
	}

	function nosmiley()
	{
	    $this->nosmiley = $this->getVar("nosmiley");
		return $this->nosmiley;
	}

	function notifypub()
	{
	    $this->notifypub = $this->getVar("notifypub");
		return $this->notifypub;
	}

	function type()
	{
	    $this->type = $this->getVar("story_type");
		return $this->type;
	}

	function ihome()
	{
	    $this->ihome = $this->getVar("ihome");
		return $this->ihome;
	}

	function topicdisplay()
	{
	    $this->topicdisplay = $this->getVar("topicdisplay");
		return $this->topicdisplay;
	}

	function topicalign($astext = true)
	{
	    $this->topicalign = $this->getVar("topicalign");
		if ($astext) {
			if ($this->topicalign == "L") {
				$ret = "left";
			} else {
				$ret = "right";
			}
	...
	function comments()
	{
	    $this->comments = $this->getVar("comments");
		return $this->comments;
	}
}
class amsams_articleHandler extends XoopsPersistableObjectHandler 
{

	function __construct(&$db) 
	{
		parent::__construct($db, "ams_article", "ams_article", "storyid", "title");
	}
}
?>