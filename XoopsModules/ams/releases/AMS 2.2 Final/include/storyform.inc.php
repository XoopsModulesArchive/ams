...
//include XOOPS_ROOT_PATH."/modules/AMS/wysiwyg/formwysiwygtextarea.php";
...
$sform->addElement(new XoopsFormText(_AMS_NW_TITLE, 'title', 50, 80, $title), true);
...
$sform->addElement(new XoopsFormLabel(_AMS_NW_TOPIC, $topic_obj_tree->makeSelBox('topic_id', 'topic_title', '--', $topicid)));

//If admin - show admin form
//TODO: Change to "If submit privilege"
if ($approveprivilege) {
    //Show topic image?
    $sform->addElement(new XoopsFormRadioYN(_AMS_AM_TOPICDISPLAY, 'topicdisplay', $topicdisplay));
    //Select image position
    $posselect = new XoopsFormSelect(_AMS_AM_TOPICALIGN, 'topicalign', $topicalign);
    ...
    $sform->addElement(new XoopsFormRadioYN(_AMS_AM_PUBINHOME, 'ihome', $ihome, _NO, _YES));
    $audience_handler =& xoops_getmodulehandler('audience', 'AMS');
    $audiences = $audience_handler->getAllAudiences();
    $audience_select = new XoopsFormSelect(_AMS_NW_AUDIENCE, 'audience', $audience);
...
$text = isset($hometext) && $hometext != "" ? $hometext : '';
$text .= isset($bodytext) && $bodytext != " "  ? "[extend]".$bodytext : "";
//$sform->addElement(new XoopsFormWysiwygTextArea(_AMS_NW_THESCOOP, 'hometext', $hometext, '100%', '100px', ''), true);
$sform->addElement(new XoopsFormDhtmlTextArea(_AMS_NW_THESCOOP, 'hometext', $text, 15, 60, 'hiddenHometext'));

/*
//Extra info
if ($approveprivilege) {
    //$sform->addElement(new XoopsFormWysiwygTextArea(_AMS_AM_EXTEXT, 'bodytext', $bodytext, '100%', '200px', ''), false);
    $sform->addElement(new XoopsFormDhtmlTextArea(_AMS_AM_EXTEXT, 'bodytext', $bodytext, 10, 60, 'hiddenBodytext'));
}
*/
$sform->addElement(new XoopsFormTextArea(_AMS_NW_BANNER, 'banner', $banner));

if ($edit && (!isset($_GET['approve']))) {
    if (!isset($change)) {
        $change = 0;
    }
    $change_radio = new XoopsFormRadio(_AMS_NW_MAJOR, 'change', $change);
    ...
    $sform->addElement(new XoopsFormRadioYN(_AMS_NW_SWITCHAUTHOR, 'newauthor', 0));
}
...
if ($approveprivilege) {
    ...
    $option_tray->addElement(new XoopsFormDateTime(_AMS_AM_SETDATETIME, 'publish_date', 15, $published));
    ...
    $option_tray->addElement(new XoopsFormDateTime(_AMS_AM_SETEXPDATETIME, 'expiry_date', 15, $expired));
}
