...
if (!isset($_POST['submit'])) {
    if (!isset($_GET['storyid'])) {
        redirect_header('index.php', 3, _AMS_NW_NOSTORY);
        exit();
    }
    ...
    if (!$xoopsUser || !$gperm_handler->checkRight("ams_approve", $story->topicid(), $xoopsUser->getGroups(), $xoopsModule->mid())) {
        redirect_header('article.php?storyid='.$story->storyid, 3, _NOPERM);
        exit();
    }
    ...
    $xoopsTpl->assign('story', $story->toArray(true, false, 0, false, true));
}
else {
    switch($_POST['op']) {
        case "setversion":
        ...
        case "delversions":
            ...
            if (!empty($_POST['ok'])) {
                ...
                redirect_header('article.php?storyid='.$story->storyid, 3, sprintf(_AMS_NW_VERSIONUPDATED, $_POST['version'].".".$_POST['revision'].".".$_POST['revisionminor']));
            }
            else {
                $version_array = explode(".", $_POST['version']);
                if (!isset($version_array[2])) {
