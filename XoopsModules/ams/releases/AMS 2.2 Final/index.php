...
if ($showclassic) {
    ...
    if ($scount > 0) {
        ...
        foreach ($sarray as $storyid => $thisstory) {
            $thisstory->uname($user_arr);
            $stories[] = $thisstory->toArray();
        }
    ...
    foreach ($alltopics as $topicid => $topic) {
        if ($xoopsModuleConfig['restrictindex'] && !in_array($topicid, $topics)) {
            unset($alltopics[$topicid]);
        }
        else {
            $allstories[$topicid] = AmsStory::getAllPublished($xoopsOption['storynum'], 0, false, $topicid, 1);
    ...
    if (count($uids) > 0) {
        ...
        foreach ($alltopics as $topicid => $topic) {
            ...
            foreach ($allstories[$topicid] as $thisstory) {
                $thisstory->uname($user_arr);
                $topicstories[] = $thisstory->toArray();
            }
            $key = findKey($smarty_topics, $topicstories[0]['posttimestamp']);
            
            $smarty_topics[$key] = array('title' => $topic['title'], 'stories' => $topicstories, 'id' => $topicid);
        }
    }
