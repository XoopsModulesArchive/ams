...
class AmsStory extends XoopsStory
{
    ...
    function getAllPublished($limit=0, $start=0, $checkRight = false, $topic=0, $ihome=0, $asobject=true, $order = 'published')
    {
    ...
    */
    ...
    function countPublishedByTopic($topicid=0, $checkRight = false)
    {
        ...
        if ( !empty($topicid) ) {
            ...
            if ($checkRight) {
                ...
                $groups = $xoopsUser ? $xoopsUser->getGroups() : XOOPS_GROUP_ANONYMOUS;
                $gperm_handler =& xoops_gethandler('groupperm');
                $topics = $gperm_handler->getItemIds('ams_view', $groups, $xoopsModule->getVar('mid'));
                $topics = implode(',', $topics);
                $sql .= " AND topicid IN (".$topics.")";
            }
        }
    ...
    function imglink($show_avatar = false)
    {
        $ret = '';
        $use_topic = true;
        if (false != $show_avatar && $this->uid() > 0) {
            ...
            if ($author->getVar('user_avatar') != "" && $author->getVar('user_avatar') != "blank.gif") {
                $ret = "<a href='".XOOPS_URL."/modules/AMS/index.php?storytopic=".$this->topicid."'><img src='".XOOPS_URL."/uploads/".$author->getVar('user_avatar')."' alt='".$this->newstopic->topic_title()."' hspace='10' vspace='10' align='".$this->topicalign()."' /></a>";
                $use_topic = false;
            }
        }
        if (false != $use_topic) {
    ...
    function uname($users = false)
    {
        ...
        if ($users != false && isset($users[$this->uid()])) {
            switch($option) {
                case 1:		// Username
                $this->uname = $users[$this->uid()]->getVar('uname');
                break;
                case 2:		// Display full name (if it is not empty)
                if ($users[$this->uid()]->getVar('name') == "") {
                    $this->uname = $users[$this->uid()]->getVar('uname');
                    break;
                }
                $this->uname = $users[$this->uid()]->getVar('name');
                break;
                case 3:		// Nothing
                $this->uname = '';
                break;
    ...
    function store($approved=false)	{
        ...
        if ( !isset($this->storyid) ) {
            ...
            $audience = ( $this->approved ) ? $this->audienceid : 1;
            $this->isNew = true;
            $sql = sprintf("INSERT INTO %s (storyid, title, created, published, expired, hostname, nohtml, nosmiley, counter, topicid, ihome, notifypub, story_type, topicdisplay, topicalign, comments, banner, audienceid) VALUES (%u, '%s', %u, %u, %u, '%s', %u, %u, %u, %u, %u, %u, '%s', %u, '%s', %u, '%s', %u)", $this->table, $newstoryid, $title, $created, $published, $expired, $this->hostname, $this->nohtml, $this->nosmiley, 0, $this->topicid, $this->ihome, $this->notifypub, $this->type, $this->topicdisplay, $this->topicalign, $this->comments, $myts->addSlashes($this->banner), $audience);
    ...
    function toArray($admin = false, $pagenav = false, $storypage = 0, $show_avatar = false, $single_name = false) {
        ...
        if ( trim($bodytext) != '' ) {
            if (false != $pagenav) {
                ...
                if ($story_pages > 1) {
                    ...
                    $pagenav = new XoopsPageNav($story_pages, 1, $storypage, 'page', 'storyid='.$this->storyid);
        ...
        $story['fullcount'] = strlen(trim($this->bodytext()));
        $story['bytestext'] = sprintf(_AMS_NW_BYTESMORE,$story['fullcount']);
        ...
        if ( $this->topicdisplay() ) {
            $story['imglink'] = $this->imglink($show_avatar);
