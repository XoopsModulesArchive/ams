...
class AmsTopic extends XoopsTopic
{
	...
	function getTopicPath($withAllLink = false)
	{
        $filename = "index.php";
		If ($withAllLink) {
		...
		if ($parentid > 0) {
			...
			$parentid = $this->topic_pid();
			$ret = $this->getTopicPath(true) . " > " .$ret;
		}
		else {
		    $ret = "<a href='".XOOPS_URL."/modules/AMS/".$filename."'>".$GLOBALS['xoopsModuleConfig']['index_name']."</a> > ".$ret;
		}
		return $ret;
	}
