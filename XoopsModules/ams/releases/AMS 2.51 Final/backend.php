...
if (@include_once XOOPS_ROOT_PATH.'/modules/AMS/class/class.newsstory.php') {
	$sarray = AmsStory::getAllPublished(10, 0);
} 	
	if ( !empty($sarray) && is_array($sarray) ) {
		foreach ($sarray as $story) {
			$tpl->append('items', array('title' => xoops_utf8_encode(htmlspecialchars($story->title(), ENT_QUOTES)), 'link' => XOOPS_URL.'/modules/AMS/article.php?storyid='.$story->storyid(), 'guid' => XOOPS_URL.'/modules/AMS/article.php?storyid='.$story->storyid(), 'pubdate' => formatTimestamp($story->published(), 'rss'), 'description' => xoops_utf8_encode(htmlspecialchars($story->hometext(), ENT_QUOTES))));
		}
		$sarray='';
	}
