...
}elseif ( isset($_GET['hometext'] ))
{
	$hometext = $_GET['hometext'];
}

if (isset($_POST['bodytext'] ))
{
	$bodytext = $_POST['bodytext'];
}elseif ( isset($_GET['bodytext'] ))
{
	$bodytext = $_GET['bodytext'];
}
...
if ( isset($_POST['preview'] ) || isset($_GET['preview'] )) {
	$op = 'preview';
} elseif ( isset($_POST['post']) || isset($_GET['post'])) {
	$op = 'post';
}
elseif ( (isset($_GET['op']) || isset($_POST['op'])) && (isset($_GET['storyid']) || isset($_POST['storyid']))) {
    if ($approveprivilege && (($_GET['op']  == 'edit') || ($_POST['op'] == 'edit'))){
        $op = 'edit';
    }
    elseif ($approveprivilege && (($_GET['op'] == 'delete') || ($_POST['op'] == 'delete'))) {
        $op = 'delete';
    }
    else {
...
case "post":
	...
	if ( empty( $_POST['storyid'] ) ) {
		...
		if ($change == 4)
		{
			...
			if ( (($myts->stripSlashesGPC($hometext))!= ($myts->stripSlashesGPC($story->hometext))) ||
			($myts->stripSlashesGPC($bodytext)!=$myts->stripSlashesGPC($story->bodytext)) ||
	...
	if ($result) {
	    ...
	    if(isset($_POST['xoops_upload_file'])&& isset($_FILES[$_POST['xoops_upload_file'][0]]))
	    {
	        ...
	        if(trim($fldname!=''))
	        {
	            ...
	            // Actually : Web pictures (png, gif, jpeg), zip, doc, xls, pdf, gtar, tar, txt, tiff, htm, xml, ico
	            $permittedtypes=array('text/xml','text/html', 'text/plain','image/tiff', 'image/vnd.microsoft.icon', 'image/gif', 'image/jpeg', 'image/pjpeg', 'image/x-png', 'image/png' ,'application/x-zip-compressed','application/msword', 'application/vnd.ms-excel', 'application/pdf', 'application/x-gtar', 'application/x-gzip', 'application/x-tar','application/zip');
