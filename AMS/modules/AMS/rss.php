...
if (!$tpl->is_cached('db:ams_rss.html')) {
    xoops_load('XoopsLocal');
    $tpl->assign('channel_title', XoopsLocal::convert_encoding(htmlspecialchars($xoopsConfig['sitename']." : ".$xoopsModule->getVar("name"), ENT_QUOTES)));
    $tpl->assign('channel_link', XOOPS_URL . '/modules/AMS/');
    ...
    if (file_exists($fileinc = $GLOBALS['xoops']->path('modules/AMS/class/class.newsstory.php'))) {
        include $fileinc;
    ...
    if (!empty($sarray) && is_array($sarray)) {
        foreach ($sarray as $story) {
            $tpl->append('items', array(
                ...
                'link' => XOOPS_URL . '/modules/AMS/article.php?storyid=' . $story->storyid() ,
                'guid' => XOOPS_URL . '/modules/AMS/article.php?storyid=' . $story->storyid() ,
